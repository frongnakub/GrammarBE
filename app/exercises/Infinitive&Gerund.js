...
export default class Gerund extends Component {
  state = {
    questions: [],
    loading: true,
    index: 0,
  ...
  fetchQuestions() {
    axios.get("http://localhost:3003/infinitiveGerundExercise")
    .then(res => {
                      ...
                      answer ?
                        <View>
                          ...
                          {questions.length === index + 1 ? (
                            <TouchableOpacity style={styles.iconContainer} onPress={() => this.props.navigation.navigate('Gerund')}>
                              <Icon
                                  name="right"
                                  color="#fff"
                                  size={20}
